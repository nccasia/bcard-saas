...
export const editTextCard1 = (e: any) => {
  const stage = e.target.getStage();
  const node = e.target as any;
  //const textPosition = node.getAbsolutePosition();
  const stageRect = stage.container().getBoundingClientRect();
  ...
  textarea.style.top = absPos.y + "px";
  textarea.style.left = absPos.x + "px";
  textarea.style.width = String(Number(nodeRect.width + 10)) + "px";
  textarea.style.height = String(Number(nodeRect.height + 10)) + "px";
  textarea.style.outlineStyle = "solid";
  textarea.style.outlineColor = "blue";
